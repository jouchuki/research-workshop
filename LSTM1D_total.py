...
def cnn(learning_rate=0.001, optimizer='Adam'):
    model = Sequential([
...
model = KerasClassifier(build_fn=cnn, epochs=50, batch_size=32, verbose=0)

search_spaces = {
    'learning_rate': Real(1e-7, 1e-3, prior='log-uniform'),
    'optimizer': Categorical(['Adam', 'SGD', 'RMSprop', 'Adagrad', 'Adadelta', 'Ftrl', 'Nadam', 'AdamW'])
    #'L2_score': Real(0.00001, 0.1),
    #'dropout_2d': Real(0.0, 0.2),
    #'dropout_dense': Real(0.0, 0.5)
}
...
best_model = cnn(**bayes_search.best_params_)
